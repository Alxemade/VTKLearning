...
#include <vtkImageIdealHighPass.h>  // 理想高通滤波器
#include <vtkImageButterworthHighPass.h>  // 巴特沃斯高通滤波器
...
int main()
{
	...
	fftCastFilter->Update();
 	// 下面是通过理想/巴特沃斯高通滤波器进行滤波， vtkImageIdealHighPass/vtkImageButterworthHighPass
	// 理想低通滤波器可能存在振铃效应，所以一般使用巴特沃斯高通滤波器
	// 低通滤波器平滑图像，高通滤波器求边缘
	vtkSmartPointer<vtkImageButterworthHighPass> lowPassFilter =
		vtkSmartPointer<vtkImageButterworthHighPass>::New();  // 高通滤波，低于某个给定频率的频率全部置为0
	lowPassFilter->SetInputConnection(fftFilter->GetOutputPort());
	lowPassFilter->SetXCutOff(0.1);  // 设置x方向的截止频率
	lowPassFilter->SetYCutOff(0.1);  // 设置y方向的截止频率
