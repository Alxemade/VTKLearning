...
#include <vtkImageIdealLowPass.h>  // 理想低通滤波器
#include <vtkImageButterworthLowPass.h>  // 巴特沃斯低通滤波器
...
int main()
{
	...
	fftCastFilter->Update();
 	// 下面是通过理想/巴特沃斯低通滤波器进行滤波， vtkImageIdealLowPass/vtkImageButterworthLowPass
	// 理想低通滤波器可能存在振铃效应，所以一般使用巴特沃斯低通滤波器
	vtkSmartPointer<vtkImageButterworthLowPass> lowPassFilter =
		vtkSmartPointer<vtkImageButterworthLowPass>::New();  // 低通滤波，高于某个给定频率的频率全部置为0
	lowPassFilter->SetInputConnection(fftFilter->GetOutputPort());
	lowPassFilter->SetXCutOff(0.05);  // 设置x方向的截止频率
	lowPassFilter->SetYCutOff(0.05);  // 设置y方向的截止频率
